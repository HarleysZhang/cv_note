...
3. CNN 网络一个典型的模块是由卷积(Conv)->批标准化(BNorm)->激活(Activ)->池化(Pool)这样的顺序操作组成的。对于异或神经网络，设计出的模块是由**批标准化(BNorm)->二值化激活(BinActiv)->二值化卷积(BinConv)->池化(Pool)**的顺序操作完成。这样做的原因是批标准化以后，保证了输入均值为0，然后进行二值化激活，保证了数据为-1或者+1，然后进行二值化卷积，这样能最大程度上减少特征信息的损失。二值化残差网络结构定义实例代码如下：
...
4. [XNOR-Nethttps://arxiv.org/abs/1603.05279]()
