...
**准确率描述了模型有多准**，即在预测为正例的结果中，有多少是真正例；**召回率则描述了模型有多全**，即在为真的样本中，有多少被我们的模型预测为正例。以查准率P为纵轴、查全率R为横轴作图，就得到了查准率－查全率曲线，简称**P-R**曲线。查准率、查全率性能的性能度量，除了平衡点（BEP），更为常用的是 **F1 度量** ，F1 计算公式如下：
$$F1 = \frac{2*P*R}{P+R} = \frac{2*TP}{样例总数+TP-TN}$$

F1度量的一般形式：$F_{\beta}$，能让我们表达出对查准率/查全率的偏见，$F_{\beta}$计算公式如下：
