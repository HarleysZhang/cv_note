...
**Pytorch 和 tensorflow 的卷积和池化函数，在valid 条件下卷积和池化都向下取整**。**在实际设计的卷积神经网络中，卷积层一般填充像素(`same`)，池化层一般不填充像素(`valid`)**？注意：`stride`为 1 的时候，当`kernel`为 3 `padding`为 1 或者`kernel`为 5 `padding`为2，这种情况可直接得出卷积前后尺寸不变。
