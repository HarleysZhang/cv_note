...
2. [pytorch-OpCounter](https://github.com/Lyken17/pytorch-OpCounter): 一键输出模型总的计算量 `MACs` 和参数量 `params`，支持输入自定义算子。

两个工具的安装都很简单，如直接 `pip install torchinfo` 即可。下面是工具使用示例，用于统计 `resnet50` 模型的参数量和计算量信息。
